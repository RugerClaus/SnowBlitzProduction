...
from application.SnowBlitz.entities.player.playermechanics import PlayerMechanics as physics
...
class Player(Entity):
    ...
    def get_pixel_diameter(self):
        return max(6,int(self.system.window.get_height()*self.diam_ratio))

    def rebuild_surface(self):
        visual_size = max(5,int(self.render_diam))
        self.base_size = visual_size / 2
        self.surface = self.system.window.make_surface(visual_size,visual_size,True)
    ...
    def update_position_from_ratio(self):
        ...
        self.y = self.system.window.get_height()*self.y_ratio
    ...
    def update(self):
        ...
        self.diam = max(self.diam,1)
        self.render_diam += (self.diam - self.render_diam) * 0.1
    ...
    def reset(self):
        ...
        self.diam = self.system.window.get_height()*self.diam_ratio

        self.diam = max(self.diam,6)
