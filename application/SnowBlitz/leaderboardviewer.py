...
from application.network.leaderboard import Leaderboard
...
class LeaderboardViewer:
    ...
    def fetch_task(self):
        ...
        try:
            ...
            log_event("Leaderboard fetch exception",str(e))
            with self.lock:
                self.cached_data = None
    ...
    def draw_header(self):
        ...
        font = self.system.font.get_font(self.font)

        username = font.render("USERNAME",True,(255,255,0))
        score = font.render("SCORE",True,(255,255,0))

        username_x = widget.rect.left + int(
            widget.surface.get_width() * self.USERNAME_X
        )

        score_x = widget.rect.left + int(
            widget.surface.get_width() * self.SCORE_X
        )

        header_y = widget.rect.top - int(
            self.system.window.get_height() * self.HEADER_OFFSET
        )

        self.system.window.blit(
            username,
            username.get_rect(left=username_x,centery=header_y)
        )

        self.system.window.blit(
            score,
            score.get_rect(left=score_x,centery=header_y)
        )
