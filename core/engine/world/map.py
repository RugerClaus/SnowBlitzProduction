...
class Map:
    ...
    def create_grid_surface(self):
        ...
        self.grid_surface = self.system.window.make_surface(
            grid_width,
            grid_height,
            True
        )
    ...
    def render_layer(self):
        ...
        layer_size = (
            round(ww * self.map_width),
            round(wh * self.map_height)
        )

        if (
            self.layer_surface is None
            or self.layer_size != layer_size
        ):
            self.create_layer_surface()
    ...
    def render_debug_grid(self):
        ...
        if (
            self.grid_surface is None
            or self.grid_size != grid_size
        ):
    ...
    def draw(self, camera=None):
        ...
        if (
            self.camera_follow_x
            and camera is not None
        ):
            screen_x -= camera_x * ww

        if (
            self.camera_follow_y
            and camera is not None
        ):
    ...
    def load_cells(self, cell_map):
        ...
        if len(cell_map) != self.rows:
            raise ValueError(
                f"Expected {self.rows} rows, "
                f"got {len(cell_map)}"
            )

        if len(cell_map[0]) != self.columns:
            raise ValueError(
                f"Expected {self.columns} columns, "
                f"got {len(cell_map[0])}"
            )
