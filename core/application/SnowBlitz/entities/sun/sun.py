...
class Sun(Entity):
    ...
    def update(self):
        ...
        sh = self.system.window.get_height()
        ...
        if progress is None:
            ...
            self.x = self.rect.centerx / sw
            self.y = self.rect.centery / sh
            return

        travel_distance = sw + self.rect.width + 200
        self.rect.centerx = sw + 100 - travel_distance * progress
        self.rect.centery = self.horizon - self.arc_height - self.rect.height * math.sin(math.pi * progress)
        self.position = self.rect.center
        self.x = self.rect.centerx / sw
        self.y = self.rect.centery / sh
