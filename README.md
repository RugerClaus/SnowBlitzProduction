...
<p>So at this point, the game is basically ready for a beta release. I just have the top 10 scores displayed on the leaderboard section of the main menu, but that's A-OK. It fetches data from the API every single time you open the leaderboard and has handling for if there's a timeout, the user is offline, or the page is down. I even added a cool little loading animation that times out after a few seconds, obcviously.</p>
